        ...
        public ActionResult Index(string res, string id)
        {
            ...
            if (Resource.Contains(res) && id != null && Request.Cookies["FHIR_url"] != null)
            {
                try
                {
                    ...
                    {
                        //e.RawRequest.Headers.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(Encoding.ASCII.GetBytes(cookies.FHIR_Token_Cookie(HttpContext)))); 有cookies_信華原本的
                        e.RawRequest.Headers.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(Encoding.ASCII.GetBytes("帳號:密碼")));
